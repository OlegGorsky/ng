...
Для работы helper-команды генерации изображений нужен Python. Если его нет, Windows setup попробует поставить Python через `winget` для текущего пользователя и продолжит настройку даже при неудачной установке.
